function [counter] = read_all_prototypes(folder,recursive,fid,counter)
...
%     % matspace.utils.read_all_prototypes(folder,recursive);
%
% Change Log:
...
for i = 3:length(list)
    ...
    if list(i).isdir
        ...
        if recursive && ~any(strcmp(list(i).name,folder_exclusions))
            counter = read_all_prototypes(this_name,recursive,fid,counter);
        else
            continue
        end
    else
