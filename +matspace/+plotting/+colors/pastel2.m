function [colors] = pastel2()

% PASTEL2  is the Pastel2 colormap from Python matplotlib.
%
% Input:
%     (None)
%
% Output
%     colors : (3x8) table of colors that are useful to plot with
%
% Prototype:
%     colors = matspace.plotting.colors.pastel2();
%     % plotting example:
%     surf(peaks);
%     colormap('matspace.plotting.colors.pastel2');
%     colorbar;
%
% See Also:
%     matspace.plotting.colors.get_python_colormap, colormap
%
% Change Log:
%     1.  Written by David C. Stauffer in January 2026.

% matspace.plotting.colors.get_python_colormap('Pastel2')
colors = [...
     0.701960784313725         0.886274509803922         0.803921568627451
     0.992156862745098         0.803921568627451         0.674509803921569
     0.796078431372549         0.835294117647059         0.909803921568627
     0.956862745098039         0.792156862745098         0.894117647058824
     0.901960784313726          0.96078431372549         0.788235294117647
                     1         0.949019607843137         0.682352941176471
     0.945098039215686         0.886274509803922                       0.8
                   0.8                       0.8                       0.8];