function map = plasma(m)

% PLASMA  Perceptually Uniform Blue-Pink-Orange-Yellow color map.
%
% Summary:
%     PLASMA(m) returns an m-by-3 matrix containing a colormap.
%
% Input:
%     m : (scalar) number of colors to produce
%
% Output:
%     map : (mx3) color triple
%
% Prototype:
%     map = matspace.plotting.colors.plasma();
%     assert(all(size(map) == [64 3]));
%     assert(min(min(map)) >= 0);
%     assert(max(max(map)) <= 1);
%     % plotting example:
%     surf(peaks);
%     colormap('matspace.plotting.colors.plasma');
%     colorbar;
%
% See Also:
%     colormap, parula, matspace.plotting.colors.get_python_colormap
%
% Change Log:
%     1.  Written by David C. Stauffer in January 2026.

if nargin < 1
    f = get(groot,'CurrentFigure');
    if isempty(f)
        m = size(get(groot,'DefaultFigureColormap'),1);
    else
        m = size(f.Colormap,1);
    end
end

% plasmi = matspace.plotting.colors.get_python_colormap('plasma', 64)
plasmi = [...
    0.050383 0.029803 0.527975
    0.096379 0.025165 0.547103
    0.132381 0.022258  0.56325
     0.16407 0.020171 0.577478
    0.193374 0.018354  0.59033
    0.221197 0.016497 0.602083
    0.248032 0.014439 0.612868
    0.274191 0.012109 0.622722
    0.299855 0.009561 0.631624
     0.32515 0.006915 0.639512
     0.35015 0.004382 0.646298
    0.374897 0.002245 0.651876
    0.399411 0.000859 0.656133
    0.423689 0.000646 0.658956
    0.447714  0.00208  0.66024
    0.471457 0.005678 0.659897
    0.500678 0.014055 0.657088
    0.523633 0.024532 0.652901
    0.546157 0.038954  0.64701
    0.568201 0.055778 0.639477
    0.589719 0.072878 0.630408
    0.610667 0.090204 0.619951
    0.631017 0.107699 0.608287
    0.650746 0.125309 0.595617
    0.669845 0.142992 0.582154
    0.688318 0.160709 0.568103
    0.706178 0.178437 0.553657
    0.723444 0.196158 0.538981
    0.740143 0.213864 0.524216
    0.756304 0.231555 0.509468
    0.771958 0.249237 0.494813
    0.787133 0.266922 0.480307
    0.805467 0.289057 0.462415
    0.819651 0.306812 0.448306
    0.833422 0.324635 0.434366
    0.846788 0.342551 0.420579
     0.85975 0.360588 0.406917
    0.872303 0.378774 0.393355
    0.884436 0.397139  0.37986
    0.896131 0.415712 0.366407
    0.907365 0.434524  0.35297
    0.918109 0.453603 0.339529
    0.928329 0.472975 0.326067
     0.93799 0.492667 0.312575
    0.947051 0.512699 0.299049
     0.95547 0.533093  0.28549
    0.963203 0.553865 0.271909
    0.970205 0.575028 0.258325
    0.977856 0.602051 0.241387
    0.983041 0.624131 0.227937
    0.987332 0.646633 0.214648
    0.990681 0.669558 0.201642
    0.993032 0.692907 0.189084
    0.994324 0.716681 0.177208
    0.994495  0.74088 0.166335
    0.993482 0.765499 0.156891
    0.991209 0.790537 0.149377
    0.987621 0.815978 0.144363
    0.982653 0.841812 0.142303
    0.976265 0.868016 0.143351
    0.968443 0.894564 0.147014
    0.959276 0.921407 0.151566
    0.949151 0.948435 0.152178
    0.940015 0.975158 0.131326];

P = size(plasmi,1);

map = interp1(1:P, plasmi, linspace(1,P,m), 'linear');