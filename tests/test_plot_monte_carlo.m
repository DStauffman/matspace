classdef test_plot_monte_carlo < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function test_with_opts(self)
            self.figs(end+1) = matspace.plotting.plot_monte_carlo(self.time, self.data, Opts=self.opts);
        end
        ...
        function test_with_everything(self)
            self.figs(end+1) = matspace.plotting.plot_monte_carlo(self.time, self.data, Opts=self.opts, Type='percentage', ...
        ...
        function test_with_bad_opts(self)
            self.verifyError(@() matspace.plotting.plot_monte_carlo(self.time, self.data, Opts=struct('case_name', 'test')), ...
                'MATLAB:InputParser:ArgumentFailedValidation');
        end

        function test_with_bad_values(self)
            self.verifyError(@() matspace.plotting.plot_monte_carlo('time', 2*sin(1:10)), ...
                'MATLAB:InputParser:ArgumentFailedValidation');
        end
    end
end