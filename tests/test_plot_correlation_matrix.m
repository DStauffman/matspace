classdef test_plot_correlation_matrix < matlab.unittest.TestCase %#ok<*PROP>
    ...
    methods (Test)
        function test_normal(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, self.labels);
        end
        ...
        function test_nonsquare(self)
            self.figs(end+1) = plot_correlation_matrix(self.data(1:5, 1:3), {self.labels(1:3), ...
                self.labels(1:5)});
        end

        function test_default_labels(self)
            self.figs(end+1) = plot_correlation_matrix(self.data(1:5, 1:3));
        end

        function test_all_args(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, self.labels, self.opts, ...
        ...
        function test_symmetric(self)
            self.figs(end+1) = plot_correlation_matrix(self.sym);
        end

        function test_symmetric_all(self)
            self.figs(end+1) = plot_correlation_matrix(self.sym, {}, [], 'LowerOnly', false);
        end

        function test_above_one(self)
            large_data = self.data * 1000;
            self.figs(end+1) = plot_correlation_matrix(large_data, self.labels);
        end

        function test_above_one_part2(self)
            large_data = self.data * 1000;
            self.figs(end+1) = plot_correlation_matrix(large_data, self.labels, [], 'cmax', 2000);
        end
        ...
        function test_below_one_part2(self)
            large_data = 1000*(self.data - 0.5);
            self.figs(end+1) = plot_correlation_matrix(large_data, self.labels, [], 'cmin', -2);
        end
        ...
        function test_within_minus_one_part2(self)
            large_data = self.data - 0.5;
            self.figs(end+1) = plot_correlation_matrix(large_data, self.labels, [], 'cmin', -1, 'cmax', 1);
        end

        function test_colormap(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, {}, [], 'colormap', 'parula');
        end

        function test_matrix_name(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, {}, [], 'MatrixName', 'Not a Correlation Matrix');
        end

        function test_plot_border(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, {}, [], 'PlotBorder', false);
        end

        function test_label_values(self)
            self.figs(end+1) = plot_correlation_matrix(self.data, {}, [], 'LabelValues', true);
        end
    end
end