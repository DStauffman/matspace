...
%     Call with mismatched arguments (should error)
    ...
    methods (Test)
        function test_no_args(self)
            out = load_check(self.filename);
            self.verifyEqual(out, self.out);
        end

        function test_with_args(self)
            [x, y] = load_check(self.filename, 'x', 'y');
            self.verifyEqual(x, self.x);
            self.verifyEqual(y, self.y);
        end
        
        function test_extra_args(self)
            [y, z] = self.verifyWarning(@() load_check(self.filename, 'y', 'z'), 'MATLAB:load:variableNotFound');
            self.verifyEqual(y, self.y);
            self.verifyEmpty(z);
        end
        
        function test_mismatch_args(self)
            [~, ~] = self.verifyError(@() load_check(self.filename, 'x', 'y', 'z'), 'load_check:IOMismatch');
        end
    end
end