classdef test_send2base < matlab.unittest.TestCase %#ok<*PROP>
    ...
    methods (Test)
        function test_simple_1(self)
            ...
            sent_vars = matspace.utils.send2base('test');
        ...
        function test_simple_2(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('name_two');
        ...
        function test_multiple(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', 'name_one');
        ...
        function test_statment_1(self)
            import matspace.utils.send2base
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            send2base test
        ...
        function test_case_sensitivity_1(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            self.verifyError(@() matspace.utils.send2base('Test'), 'matspace:BadSendVarsName');
        end
        ...
        function test_exclude_1(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', '-exclude', 'name*');
            self.verifyEqual(sent_vars, {'test'});
            self.verifyTrue(evalin('base', 'test == 123;'));
        end
        ...
        function test_exclude_2(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('-exclude', 'name*', 'test');
            self.verifyEqual(sent_vars, {'test'});
            self.verifyTrue(evalin('base', 'test == 123;'));
        end
        ...
        function test_exclude_3(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', '-exclude', 'test');
            self.verifyEmpty(sent_vars);
        end

        function test_exclude_4(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', '-exclude', 'name*', 'name_one');
            self.verifyEqual(sent_vars, {'test'});
            self.verifyTrue(evalin('base', 'test == 123;'));
        end
        ...
        function test_exclude_5(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', '-exclude', {'tested', 'name*'});
            self.verifyEqual(sent_vars, {'test'});
            self.verifyTrue(evalin('base', 'test == 123;'));
        end
        ...
        function test_exclude_6(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('test', 'name_one', 'name_two', '-exclude', {'test*', 'name_one'});
            self.verifyEqual(sent_vars, {'name_two'});
            self.verifyTrue(evalin('base', 'strcmp(name_two, ''2'');'));
        end
        ...
        function test_exclude_everything(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            sent_vars = matspace.utils.send2base('*', '-exclude', '*');
            self.verifyEmpty(sent_vars);
        end

        function test_bad_option(self)
            test = self.test; name_one = self.name_one; name_two = self.name_two; %#ok<NASGU>
            self.verifyError(@() matspace.utils.send2base('Test', '-exclusions', 'name*'), 'matspace:BadSendVarsOption');
        end
