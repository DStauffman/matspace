    ...
    methods (Test)
        function test_all_args(self)
            [figs, err] = matspace.plotting.general_quaternion_plot(self.description, self.time_one, ...
                self.quat_one, self.time_two, self.quat_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
        ...
        function test_datetime(self)
            ...
            disp_xmax = NaT;
            figs = matspace.plotting.general_quaternion_plot(self.description, time_one, self.quat_one, ...
                time_two, self.quat_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
        ...
        function test_non_overlapping(self)
            ...
            make_subplots = false;
            show_extra    = false;
            [figs, err] = matspace.plotting.general_quaternion_plot(self.description, time_one, self.quat_one, ...
                time_two, self.quat_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
                'TimeUnits', self.time_units, 'StartDate', self.start_date, 'RmsXmin', self.rms_xmin, ...
                'RmsXmax', self.rms_xmax, 'DispXmin', self.disp_xmin, 'DispXmax', self.disp_xmax, ...
                'PlotComp', self.plot_components, 'FigVisible', self.fig_visible, 'MakeSubplots', make_subplots, ...
                'SingleLines', self.single_lines, 'UseMean', self.use_mean, 'PlotZero', self.plot_zero, ...
                'ShowRms', self.show_rms, 'LegendLoc', self.legend_loc, 'ShowExtra', show_extra, ...
                'TruthName', self.truth_name, 'TruthTime', self.truth_time, 'TruthData', self.truth_data, ...
                'Tolerance', self.tolerance);
            self.figs = [self.figs, figs];
            self.verifyEqual(err.one, nan(4, 1));
            self.verifyEqual(err.two, nan(4, 1));
            self.verifyEqual(err.diff, nan(3, 1));
        end

        function test_non_overlapping_datetime(self)
            date_zero      = datetime('now');
            dt             = duration(0, 1, 0); % 1 minute
