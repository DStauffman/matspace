    ...
    methods (Test)
        function test_all_args(self)
            [figs, err] = matspace.plotting.general_difference_plot(self.description, self.time_one, ...
                self.data_one, self.time_two, self.data_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
        ...
        function test_minimal_args(self)
            figs = matspace.plotting.general_difference_plot(self.description, self.time_one, self.data_one, ...
                zeros(1,0), zeros(size(self.data_one, 1), 0));
            self.figs = [self.figs, figs];
        end
        ...
        function test_datetime(self)
            ...
            disp_xmax      = NaT;
            figs = matspace.plotting.general_difference_plot(self.description, time_one, self.data_one, ...
                time_two, self.data_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
                'Elements', self.elements, 'Units', self.units, 'TimeUnits', self.time_units, ...
        ...
        function test_non_overlapping(self)
            ...
            make_subplots = false;
            show_extra    = false;
            [figs, err] = matspace.plotting.general_difference_plot(self.description, time_one, self.data_one, ...
                time_two, self.data_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
                'Elements', self.elements, 'Units', self.units, 'TimeUnits', self.time_units, ...
                'LegendScale', self.leg_scale, 'StartDate', self.start_date, 'RmsXmin', self.rms_xmin, ...
                'RmsXmax', self.rms_xmax, 'DispXmin', self.disp_xmin, 'DispXmax', self.disp_xmax, ...
                'FigVisible', self.fig_visible, 'MakeSubplots', make_subplots, 'SingleLines', self.single_lines, ...
                'ColorOrder', self.colororder, 'UseMean', self.use_mean, 'PlotZero', self.plot_zero, ...
                'ShowRms', self.show_rms, 'LegendLoc', self.legend_loc, 'ShowExtra', show_extra, ...
                'SecondYScale', self.second_y_scale, 'YLabel', self.y_label, 'TruthName', self.truth_name, ...
                'TruthTime', self.truth_time, 'TruthData', self.truth_data, 'Tolerance', self.tolerance);
            self.figs = [self.figs, figs];
            self.verifyEqual(err.one, nan(2, 1));
        ...
        function test_non_overlapping_datetimes(self)
            date_zero      = datetime('now');
            dt             = duration(0, 1, 0); % 1 minute
            time_one       = date_zero + dt * (self.time_one - 100);
            time_two       = date_zero + dt * (self.time_two + 100);
            rms_xmin       = date_zero + 1*dt;
            rms_xmax       = date_zero + 10*dt;
            disp_xmin      = date_zero - 2*dt;
            disp_xmax      = date_zero + 105*dt;
            make_subplots  = false;
            show_extra     = false;
            figs = matspace.plotting.general_difference_plot(self.description, time_one, self.data_one, ...
                time_two, self.data_two, 'NameOne', self.name_one, 'NameTwo', self.name_two, ...
