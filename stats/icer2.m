function [inc_cost, inc_qaly, icer_out, order] = icer2(cost, qaly)

% ICER2  calculates the incremental cost effectiveness ratios compared to the first given strategy.
%
% Summary:
%     This function is very similar to icer.m, but assumes the first strategy is never dominated.
...
%     qaly = [20; 30; 40; 80];
%     [inc_cost, inc_qaly, icer_out, order] = icer2(cost, qaly);
%     % test results
...
while true
    ...
    % sort by cost
    [sorted_cost, ix_sort] = sort(this_cost(2:end));
    sorted_qaly = this_qaly(ix_sort + 1);
    ...
    inc_cost = [this_cost(1); sorted_cost(1)-this_cost(1); diff(sorted_cost)];
    inc_qaly = [this_qaly(1); sorted_qaly(1)-this_qaly(1); diff(sorted_qaly)];
    icer_out = inc_cost ./ inc_qaly;

    % check for weakly dominated strategies
    if ~issorted(icer_out(2:end))
